...
namespace n_hooks {
	bool __fastcall n_functions::SetupBones( std::uintptr_t ecx, std::uintptr_t edx, void* bone_to_world_out, int max_bones, int bone_mask, float CurrentTime ) {

		C_CSPlayer* player = reinterpret_cast < C_CSPlayer* >( std::uintptr_t( ecx ) - 0x4 );

		auto OriginalFn = g_animations.track[ player->Networkable( )->EntIndex( ) ].renderable->get_original_function< decltype( &SetupBones ) >( 13 );

		if ( player && player->IsAlive( ) ) {
			//if ( bone_to_world_out != nullptr ) {
			//	static float last_real_time = 0.f;

			//	if ( Interfaces::GlobalVarsBase->real_time >= last_real_time + 1.f )
			//		last_real_time = Interfaces::GlobalVarsBase->real_time;
			//}
		...
		return OriginalFn( ecx, edx, bone_to_world_out, max_bones, bone_mask, CurrentTime );
	}
}